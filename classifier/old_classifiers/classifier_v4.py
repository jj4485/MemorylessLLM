...
class GPT2WithBinaryClassifier(GPT2LMHeadModel):
    def __init__(self, config):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(num_logits, num_logits // 256),  # First hidden layer
            nn.ReLU(),  # Activation function
            nn.Linear(num_logits // 256, num_logits // 1024),  # Second hidden layer
            nn.ReLU(),  # Activation function
            nn.Linear(num_logits // 1024, 1)  # Output layer
        )

    def forward(self, input_ids, attention_mask=None, labels=None, binary_labels=None, ratio=0.5):
        # Forward pass through the GPT-2 model, getting hidden states and logits
        ...
        # Use advanced indexing to select the logits for the last non-padding token
        batch_indices = torch.arange(logits.size(0), device=logits.device)  # Create batch indices
        last_logits = logits[batch_indices, last_token_indices, :]  # Select last non-padding token's logits
        
        # Apply softmax to the last logits
        last_logits_softmax = F.softmax(last_logits, dim=-1)

        # Compute classification logits using the classifier
        cls_logits = self.classifier(last_logits_softmax)  # Apply classifier to the softmaxed logits
        ...
        if binary_labels is not None:
            # Detach last_logits_softmax to prevent gradient flow through LM
...
def main(args):
    ...
    for step, batch in enumerate(tqdm(train_dataloader, total=total_steps)):
        ...
        if step % (total_steps // args.num_evals_per_epoch) == 0:
            ...
            print(f'Step {step}/{total_steps} - Validation Loss: {val_loss}, Validation Binary Loss: {val_binary_loss}, Validation Adversarial Loss: {val_adversarial_loss}')
            report = True
        train_batch(model, batch, optimizer_lm, optimizer_cls, scheduler_lm, device, args.alpha, args.ratio, report=report)
